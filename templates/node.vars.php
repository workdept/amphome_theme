...
function amphome_preprocess_node(&$vars, $hook) {
  if ($vars['type'] == 'news_item') {
    if (isset($vars['field_news_term']) && count($vars['field_news_term']) === 1 && isset($vars['field_news_term'][0]['taxonomy_term']->field_term_image)) {
      $field_news_term = $vars['field_news_term'][0];
    }
    if (isset($vars['field_news_term']) && count($vars['field_news_term']['und']) === 1 && isset($vars['field_news_term']['und'][0]['taxonomy_term']->field_term_image)) {
